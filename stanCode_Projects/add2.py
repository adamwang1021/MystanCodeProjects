...
def add_2_numbers(l1: ListNode, l2: ListNode) -> ListNode:
    ...
    while cur_l1 is not None:
        lst_l1.append(cur_l1.val)
        cur_l1 = cur_l1.next
    cur_l2 = l2
    lst_l2 = []
    while cur_l2 is not None:
        lst_l2.append(cur_l2.val)
        cur_l2 = cur_l2.next

    l1_num = 0
    for i in range(len(lst_l1) - 1, -1, -1):
        l1_num += lst_l1[i] * (10 ** i)
    l2_num = 0
    for i in range(len(lst_l2) - 1, -1, -1):
        l2_num += lst_l2[i] * (10 ** i)
    total = l1_num + l2_num

    ans = ListNode(total % 10, None)
    total = total // 10
    cur = ans
    while True:
        if total == 0:
